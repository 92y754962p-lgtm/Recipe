    ...
    if st.button("Fetch and Build", use_container_width=True):
        with st.status("Structuring recipe...", expanded=True) as status:
            try:
                ...
                3. A list of 3 conversion options (e.g., ['700g', '25oz', '1.5lbs']).
                ...
                st.session_state.is_loaded = True # Triggers the view switch
                st.rerun()
            except Exception as e:
                st.error(f"Error: {e}")

# --- Recipe View (Only appears when is_loaded is True) ---
if st.session_state.is_loaded:
    ...
    for ing in curr["ingredients"]:
        ...
        c1.checkbox(ing["name"], key=f"chk_{st.session_state.step}_{ing['name']}")
